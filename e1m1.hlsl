...
// Windows terminal version of this old shadertoy shader
...
float noise2D( vec2 p, float r )
{
    ...
    float h00 = hash2D( fmod(fl + vec2(0.0, 0.0), r) );
    float h10 = hash2D( fmod(fl + vec2(1.0, 0.0), r) );
    float h01 = hash2D( fmod(fl + vec2(0.0, 1.0), r) );
    float h11 = hash2D( fmod(fl + vec2(1.0, 1.0), r) );
...
vec3 TexFSky1(vec2 vTexCoord, float fRandom, float fHRandom)
{
    ...
    return vec3(fShade,fShade,fShade);
}
...
vec3 TexFlat5_5( vec2 vTexCoord, float fRandom )
{
    ...
    col *= fmod(vTexCoord.x, 2.0) * 0.15 + 0.85;
...
vec3 TexCeil3_5( vec2 vTexCoord, float fRandom )
{
    ...
    if( (vTileCoord.x >= 0.0) && (vTileCoord.x < 32.0) ) 
        ...
        vTileCoord.y -= 11.0;
    vTileCoord.x = fmod(vTileCoord.x, 32.0);
    vTileCoord.y = fmod(vTileCoord.y, 64.0);
    ...
    {
        float fRRow = floor(fmod(vTileCoord.y - 3.5, 7.5));
        float fRColumn = fmod(vTileCoord.x - 15.0, 10.0);
...
vec3 TexDoor3(vec2 vTexCoord, float fRandom, float fHRandom)
{
    ...
    {
        vec2 vRepeatCoord = fmod( vTexCoord, vec2( 8.0, 48.0 ) );
...
vec3 TexLite3( vec2 vTexCoord )
{
    ...
    vLocalCoord.y = fmod(vLocalCoord.y, 8.0 );
...
vec3 TexBrown1( vec2 vTexCoord, float fRandom, float fHRandom )
{
    ...
    vRepeatCoord.x = fmod(vRepeatCoord.x, 13.0);
...
void DrawScreen(inout vec3 col, const in vec2 vTexCoord, const in vec2 vPos, const in vec2 vSize, const in vec3 vCol)
{
    ...
    col *= Indent( vScreenCoord, vSize, vec2(-1.0,-1.0), 1.2, 0.5);
...
vec3 TexCompute2( vec2 vTexCoord, float fRandom )
{
    ...
    {
        vec2 vLocalCoord = vTexCoord;
        vLocalCoord.x = fmod(vLocalCoord.x, 21.0);
    ...
    if(vTexCoord.y < 40.0)
    {
        ...
        vec3 vCol = vec3(0.0,0.0,0.0);
        float iIndex = floor(fmod(fIndex, 5.0));
        if( iIndex < 0.5 ) 
        {
        ...
        {
            vCol = mix(vec3(1.0, 0.6, 0.02), vec3(0.1,0.1,0.1), fRandom);
        }
        else if(iIndex < 2.5)
        {
            vCol = vec3(fRandom * 0.5,fRandom * 0.5,fRandom * 0.5);
        }
        else if(iIndex < 3.5)
        {
            vCol = vec3(fRandom * 0.25,fRandom * 0.25,fRandom * 0.25);
        }
        else if(iIndex < 4.5)
        {
            vCol = mix(vec3(0.0, 0.0, 0.5), vec3(0.1,0.1,0.1), fRandom);
        }
...
vec3 TexSupport2( vec2 vTexCoord, float fRandom )
{
    ...
    if((vLocalCoord.y < 64.0) || (vLocalCoord.y > 75.0))
    {
        ...
        vLocalCoord = fmod( vLocalCoord, vec2(20.0, 16.0));
        float fIndent = Indent( vLocalCoord, vec2(8.0, 8.0), vec2(16.0, 15.0), 0.9, 1.1);
        fShade += fIndent - 1.0;
    }
    
    col = vec3(fShade,fShade,fShade);
...
vec3 TexDoorTrak( vec2 vTexCoord, float fRandom )
{
    ...
    fShade *= fmod(vTexCoord.x, 2.0) * 0.6 + 0.4;
    return vec3(fShade,fShade,fShade);
}
...
vec3 SampleTexture( const in float fTexture, const in vec2 _vUV )
{
    ...
    else if(fTexture == TEX_STARTAN3) { vSize = vec2(128.0,128.0); fPersistence = 1.0; }    
    else if(fTexture == TEX_BROWN1) { vSize = vec2(128.0,128.0); fPersistence = 0.7; }  
